...
class Box:
    ...
    center_left, center_upper: int
        The coordinates of the cener of the box with respect to the form.
    img: object
    ...
    def find_left_upper_corner(self, crop_img, tresh=100):
        ...
        for l in np.argsort(np.sum(data[:, :width/2], axis=0))[-5:][::-1]:
            for u in np.argsort(np.sum(data[:height/2, :], axis=1))[-5:][::-1]:
                val = min(np.sum(data[u:u+Box.length_box, l]),
    ...
    def mark_position(self, img, color=0, lw=4):
        ...
        for i in range(lw):
            draw.rectangle([(self.left+i,
                             self.upper+i),
                            (self.left+Box.length-i,
                             self.upper+Box.length-i)],
                           outline=color)
        del draw
